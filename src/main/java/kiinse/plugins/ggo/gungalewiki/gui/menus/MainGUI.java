...
public class MainGUI extends CreatedGui {
    ...
    public MainGUI() {
        ...
        this.filtersButtons = getGunGaleWiki().getFilterButtons();
    ...
    public void onOpenInventory(@NotNull GunGaleWiki gunGaleWiki) {
        setCreatedItem(new PlayerHead(getPlayer()));
        for (var pos : new int[]{0, 1, 2, 9, 10, 11}) {
            setCreatedItem(new LastCraftsButton(getPlayerLocale(), gunGaleWiki, this, pos));
        }
        for (var pos : new int[]{6, 7, 8, 15, 16, 17}) {
            setCreatedItem(new BookMarkButton(getPlayerLocale(), gunGaleWiki, this, pos));
        }
